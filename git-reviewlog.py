...
class GitLogReviewer:
    ...
    __sortKey = None
    __endFlag = False
    __gitCmd = 'ssh -p 29418 gerrit.sh.cn.ao.ericsson.se gerrit query --format=JSON --patch-sets --all-approvals status:closed project:\"MA/ma\"'
    ...
    def parseRecord(self, record):
        ...
        for subItem in lastItem['approvals']:
            ...
            if subItem['type'] == 'CRVW':
                ...
                revieweBy = subItem['by']['name']
            if subItem['type'] == 'SUBM':
                submitDate = datetime.datetime.fromtimestamp(int(subItem['grantedOn']))
        
        self.__resultList.append("%s, %s, %d, %d, %d, %s, %s, %s, %s, %s, %s" %(id, name, number, fail1, fail2, createDate, lastCommitCreateDate, lastSCMVerifyDate, reviewDate, submitDate, revieweBy))
        #print '4'
        return 0
        ...
        try:
            ...
            if newFlag:
                ...
                if path != '' and not os.path.exists(path):
                    os.makedirs(path)
                fh = open(filePath, 'w')
                fh.write('Id,  Owner,  PatchsetNumber,  -1 Votes,  -2 Votes,  FirstCommitCreateDate, LastCommitCreateDate, LastSCMVerifyDate, ReviewDate, SubmitDate, Reviewer\n')
            else:
                fh = open(filePath, 'a+')
...
if __name__ == '__main__':
    ...
    reviewer.loopProcessCmd('/proj/EMAPublic/personal/eweitan/ma70/.git/config', 'result_loop_cfg_061203.csv')
    #reviewer.processCmd('/proj/EMAPublic/personal/exuyufe/ma_repo/.git/config', 'result_loop_cfg_new2.csv')
